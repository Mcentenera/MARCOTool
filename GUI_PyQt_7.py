...
from PyQt5.QtWidgets import (
    ...
    QTextEdit, QSlider, QCheckBox, QDoubleSpinBox, QHBoxLayout, QFrame
)
...
from src.utils import print_results, snr_cal, multi_criteria
import numpy as np
import io
import contextlib
import os
...
class MarcotApp(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("MARCOT HR Estimator v1.7")
        ...
        self.telescope_aperture_label = QLabel("Telescope efective aperture (m):")
        ...
        self.seeing_spin.setValue(1.20)
        ...
        self.pixel_size_um_spin.setValue(3.5)
        ...
        self.beam_size_mm_spin.setValue(100)
        ...
        self.grooves_mm_spin.setValue(31.5)
        ...
        advanced_layout.addWidget(QLabel("Magnification factor"), 0, 0)
        advanced_layout.addWidget(self.mag_factor_spin, 0, 1)
        advanced_layout.addWidget(self.mag_locked, 0, 2)

        advanced_layout.addWidget(QLabel("Pixel size (µm)"), 1, 0)
        advanced_layout.addWidget(self.pixel_size_um_spin, 1, 1)
        advanced_layout.addWidget(self.pixel_size_locked, 1, 2)
        ...
        advanced_layout.addWidget(QLabel("Grooves per mm"), 2, 0)
        advanced_layout.addWidget(self.grooves_mm_spin, 2, 1)
        advanced_layout.addWidget(self.grooves_mm_locked, 2, 2)
    ...
    def run_calculations(self):
        try:
            ...
            # Colores alternados para results_table
            for i, (key, value) in enumerate(results.items()):
                val_str = np.array2string(value, precision=3) if isinstance(value, (list, np.ndarray, np.generic)) else str(value)
            ...
            if isinstance(snr_fraction, np.ndarray):
                snr_fraction = snr_fraction.item() if snr_fraction.size == 1 else snr_fraction[0]
            output_text += f"\nSNR Fraction: {snr_fraction:.3f}\n"
            
            # with contextlib.redirect_stdout(buffer):
            multi_criteria("data/results_marcot.csv")
            ...
            if os.path.exists("data/score_total.csv"):
                df = pd.read_csv("data/score_total.csv", sep='\t')
                ...
                criterios = [
                    "Expected efficiency (%)", "Cost telescope + PL (MEUR)", "Total modules", "Resolution with commercial fibers",
                    "Number of OTA for high efficiency", "Spectrograph volume (m³)", "SNR fraction",
                    "OTA diameter (m)", "Recalculated module diameter (m)", "Recalculated telescope aperture (m)"
                ]
                ...
                self.best_table.setItem(row_counter, 0, QTableWidgetItem("score_total"))
                if isinstance(score_total, str) and score_total.startswith('['):
                    score_array = np.fromstring(score_total.strip('[] '), sep=' ')
