...
class MarcotApp(QWidget):
    def __init__(self):
        ...
        self.d_core_out_locked.setChecked(True)
        ...
        self.criteria_table.setColumnCount(2)
        self.criteria_table.setHorizontalHeaderLabels(["Criteria", "Weight"])
        self.criteria_table.setRowCount(10)
        ...
        for i, (crit, peso) in enumerate(initial_criteria):
            self.criteria_table.setItem(i, 0, QTableWidgetItem(crit))
            self.criteria_table.setItem(i, 1, QTableWidgetItem(str(peso)))
    ...
    def add_criteria_row(self):
        ...
        self.criteria_table.setItem(row, 1, QTableWidgetItem("1.0"))
    ...
    def get_criteria_dict(self):
        ...
        for row in range(self.criteria_table.rowCount()):
            ...
            weight_item = self.criteria_table.item(row, 1)
            if name_item and weight_item:
                try:
                    weight = float(weight_item.text())
                except ValueError:
                    weight = 1.0
                criteria[name_item.text()] = weight
        return criteria
    ...
    def run_calculations(self):
        try:
            ...
            if isinstance(snr_fraction, np.ndarray):
                snr_fraction[0] = snr_fraction[0].item() if snr_fraction[0].size == 1 else snr_fraction[0]
            output_text += f"\nSNR Fraction: {snr_fraction[0]:.3f}\n"
            
            # with contextlib.redirect_stdout(buffer):
            criteria = self.get_criteria_dict()
            multi_criteria("data/results_marcot.csv", criteria = criteria)
